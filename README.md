...
> 128GB RAM の Apple Silicon Mac 上で **200B〜700B クラスの巨大 MoE**（DeepSeek-V4 / Kimi / Qwen3-Max 等）を Streaming MoE で実用速度で動かす MLX 推論エンジン

[ElfMoon4](../ElfMoon4) のフォーク。ElfMoon4 が「24GB で 80B」を狙うのに対し、ElfMoon128 は **128GB でも載り切らないモデル** を動かすことをコンセプトとする。

OpenAI 互換 API サーバーと対話 CLI を同梱。Claude Code / opencode 等のツールからも直接利用できる。

ElfMoon は全 expert を GPU に載せるのではなく、アクティブな expert だけを SSD からストリーミングロードする。
ホットな expert は LRU キャッシュに保持し、128GB 環境では ElfMoon4 より大幅に大きいキャッシュ容量を前提とする。
...
- **超大規模 MoE 対応**: 200B〜700B クラスの expert 分解モデルを、128GB 統合メモリ + 高速 NVMe のストリーミングで動作させる。
- **デュアルモード推論**: ストリーミング MoE と `mlx_lm` 経由のオンメモリモデルの両方を同じ `chat.py` / `api_server.py` で透過的に扱える。
- **大容量 LRU キャッシュ**: 128GB を活かし、常駐 expert 数を ElfMoon4 比で大幅に拡大。ヒット率向上によりデコード速度を稼ぐ。
- **Python/MLX スタック**: コンパイル不要で即利用可能。`@mx.compile` JIT や MLX の自動 Metal ディスパッチを活用。
- **エキスパートグループ化プリフィル**: プリフィル時に同一エキスパートにルーティングされたトークンをまとめて計算する独自最適化。

> **注意**: 現時点では ElfMoon4 からのコピー直後であり、128GB 向けのチューニング（キャッシュ容量既定値、対応モデル定義、検証結果）は未実施。
